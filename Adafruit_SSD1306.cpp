...
void Adafruit_SSD1306::begin(uint8_t vccstate, uint8_t i2caddr) {
  ...
  if (sid != -1){
    ...
    if (hwSPI){
        ...
        SPI.setClockDivider(SPI_CLOCK_DIV2);	// 72MHz / 8 = 9Mhz
        SPI.setDataMode(0);
        SPI.begin();
    	}
    }
...
void Adafruit_SSD1306::shiftDisplayLeft(){
  uint8_t y;
  register uint8_t *pBuf = buffer;

  for (y=0; y<SSD1306_LCDHEIGHT/8; y++) {
    memmove(pBuf, pBuf+1, SSD1306_LCDWIDTH-1);
    pBuf += SSD1306_LCDWIDTH;
  }
}

void Adafruit_SSD1306::shiftDisplayDown(uint8_t x, uint8_t width){
  uint8_t index;
  uint8_t y;
  register uint8_t *pBuf = buffer;
...
void Adafruit_SSD1306::dbg(uint8_t *pBuf){
  Serial.print("pBuf=");
  Serial.print((long)pBuf);
  Serial.print(" : ");
  uint8_t i;
  for (i=8; i>0; i--) {
    Serial.print(*pBuf&(1<<i)>0?"1":"0");
  }
  Serial.print("\n");
}
*/
